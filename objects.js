...
SpriteMorph.prototype.drawNew = function () {
    ...
    if (this.costume && !isLoadingCostume) {
        ...
        // determine the new relative origin of the rotated shape 确定旋转形状的新相对原点
        shift = new Point(0, 0).rotateBy(
            radians(-(facing - 90)),
            pic.center()
        ).subtract(origin);

        // create a new, adequately dimensioned canvas 创建一个新的、充分的尺寸的画布
        // and draw the costume on it 然后把服装画在上面
...
SpriteMorph.prototype.forward = function (speed,distance) {
    var dest,
        dist = speed * this.parent.scale || 0;
...
//SpriteMorph.prototype.turnLeft = function (speed,angle) {
//  this.setHeading(this.heading + (+angle || 0));
//};
SpriteMorph.prototype.turn = function (speed,angle) {
    this.setHeading(this.heading + (+angle || 0));
};

SpriteMorph.prototype.start = function () {
};
...
StageMorph.prototype.dimensions =new Point(360, 360);// unscaled extent
...
StageMorph.prototype.stepGenericConditions = function (stopAll) {
    ...
    if (!hatCount) {
        ...
        ide = this.parentThatIsA(IDE_Morph);
        if (ide) {
//          ide.corralBar.stopButton.refresh();
        }
    }
};
...
// StageMorph keyboard events  键盘事件
...
// StageMorph block templates   块模板
