...
SymbolMorph.prototype.symbolCanvasColored = function (aColor) {
	...
	case 'open':
		return this.drawSymbolOpen(canvas, aColor);
	case 'submit':
		return this.drawSymbolSubmit(canvas, aColor);
    default:
        return canvas;
    }
};
...
//      l = Math.max(w / 12, 1),
	...
	ctx.beginPath();
	ctx.arc(h/2,h/2,h/2-1,0,360,false);
	ctx.lineWidth=2;
	ctx.strokeStyle = color.toString();
	ctx.closePath();
	ctx.stroke();
	...
	ctx.lineWidth=2;
	ctx.moveTo(h/3,h/3);
	ctx.lineTo(h/3,h*2/3);
	ctx.lineTo(h*2/3,h/2);
	ctx.closePath();
	ctx.stroke();
//  ctx.lineWidth = l;
//  ctx.strokeStyle = color.toString();
...
SymbolMorph.prototype.drawSymbolSave= function (canvas, color) {
    ...
    ctx.lineTo(0, 0);
    ctx.moveTo( 4 * dx, 0);
    ctx.lineTo(  4 * dx, 8 * dy);
    ctx.lineTo( 12 * dx, 8 * dy);
    ctx.lineTo( 12 * dx, 0);
    ctx.moveTo( 6 * dx, 11 * dy);
    ctx.lineTo( 6 * dx,  13 * dy);
    ctx.lineTo( 10 * dx,  13 * dy);
    ctx.lineTo( 10 * dx,  11 * dy);
    ctx.lineTo( 6 * dx,  11 * dy);
    ctx.fillStyle = color.toString();
    ctx.fill() //背景颜色填充
    ctx.stroke();
    return canvas;
}
SymbolMorph.prototype.drawSymbolOpen= function (canvas, color) {
	 var ctx = canvas.getContext('2d'),
		w = canvas.width,
	    h = canvas.height;
    ctx.strokeStyle='#373737';
	ctx.beginPath();
	ctx.moveTo(0,h);//开始绘制点坐标
	ctx.lineTo(0,0);
	ctx.lineTo(w/3,0);//第二个连接点坐标
	ctx.lineTo(w*4/9,h/5);
	ctx.lineTo(w*3/4,h/4);//第三个连接点坐标
	ctx.lineTo(w*3/4,h/2);////第四个连接点坐标
	ctx.moveTo(0,h);
	ctx.lineTo(w/4,h/2);
	ctx.lineTo(w,h/2);
	ctx.lineTo(w*3/4,h);
...
SymbolMorph.prototype.drawSymbolSubmit= function (canvas, color) {
	var ctx = canvas.getContext('2d'),
	w = canvas.width,
    h = canvas.height;;
	ctx.font = " 12px New";
