...
var PaletteHandleMorph;//面板处理
var CamSnapshotDialogMorph;//凸轮快照对话框
var num=0;
// IDE_Morph ///////////////////////////////////////////////////////////
...
IDE_Morph.prototype.setDefaultDesign = function () {
    ...
    SpriteMorph.prototype.paletteColor = new Color(169,169,169);//颜色
    ...
    IDE_Morph.prototype.backgroundColor = new Color(40, 40, 40);//背景颜色
    IDE_Morph.prototype.frameColor = SpriteMorph.prototype.paletteColor;

    IDE_Morph.prototype.groupColor
        = SpriteMorph.prototype.paletteColor.lighter(8);
    IDE_Morph.prototype.sliderColor = SpriteMorph.prototype.sliderColor;
    IDE_Morph.prototype.buttonLabelColor = new Color(255, 255, 255);//按钮字体颜色
...
IDE_Morph.prototype.scriptsTexture = function () {//脚本结构
    var pic = newCanvas(new Point(100, 100)), // bigger scales faster 更大尺度更快
...
// IDE_Morph instance creation: 实例创建:
...
IDE_Morph.prototype.init = function (isAutoFill) {
    ...
    this.logoURL = this.resourceURL('titleshubot.png');
    //this.logoURL=this.resourceURL('favicon.ico');
    this.logo = null;
    ...
    this.paletteWidth = 200; // initially same as logo width  最初与logo宽度相同
    this.stageRatio = 1; // for IDE animations, e.g. when zooming  对于IDE动画，例如缩放

	this.wasSingleStepping = false; // for toggling to and from app mode  从应用模式切换到应用模式

    this.loadNewProject = false; // flag when starting up translated  开始翻译时的标志
    this.shield = null;
...
IDE_Morph.prototype.openIn = function (world) {
    ...
    // get persistent user data, if any  获取持久的用户数据，如果有的话
    ...
    function getURL(url) {
        try {
            ...
            myself.showMessage('unable to retrieve project');//无法检索项目
            return '';
        }
    }
    ...
    // dynamic notifications from non-source text files 来自非源文本文件的动态通知
    // has some issues, commented out for now 有一些问题，现在已经被注释掉了
    /*
    ...
    */
    ...
    function interpretUrlAnchors() {//解释Url锚
        var dict, idx;
            ...
            SnapCloud.getPublicProject(
                ...
                function (projectData) {
                    ...
                    myself.nextSteps([
                        ...
                        function () {nop(); }, // yield (bug in Chrome) 收益率(Chrome错误)
                        function () {
...
IDE_Morph.prototype.buildPanes = function () {//构建窗格
    this.createLogo();
...
//  this.createCorralBar();
    this.createCorral();
    //this.createCorralright();
};

IDE_Morph.prototype.createLogo = function () {//创建Logo
    var myself = this;
    ...
    this.logo.drawNew = function () {
        ...
        var context = this.image.getContext('2d'),
            gradient = context.createLinearGradient(//创建线性渐变
                0,
                0,
                this.width(),
                0
        ...
        context.fillRect(0, 0, this.width(), this.height());//填充
        if (this.texture) {
            this.drawTexture(this.texture);
        }
    };

    this.logo.drawCachedTexture = function () {//画出缓存结构
        var context = this.image.getContext('2d');
        context.drawImage(
            this.cachedTexture,
            5,
    ...
    this.logo.mouseClickLeft = function () {//鼠标点击左
        myself.snapMenu();
    };

    this.logo.color = new Color();
    this.logo.setExtent(new Point(200, 40)); // dimensions are fixed  尺寸是固定的
    this.add(this.logo);
};

IDE_Morph.prototype.createControlBar = function () {//控制杆
    // assumes the logo has already been created 假设logo已经被创建了
    var padding = 5,
        button,//按钮,
        saveButton,
        slider,//滑块,
...
//      pauseButton,//暂停按钮
        ...
        projectButton,//项目按钮,
        settingsButton,//设置按钮
        stageSizeButton,//舞台大小按钮
        appModeButton,//App模式按钮
//      steppingButton,//步进按钮
//      cloudButton,//云按钮
        openButton,
        submitButton,
    ...
    //smallStageButton 小舞台按钮
//  button = new ToggleButtonMorph(
//      null, //colors,
//      myself, // the IDE is the target  IDE是目标
//      'toggleStageSize',
//      [
...
//  button.labelMinExtent = new Point(36, 18);//大小
//  button.padding = 0;
//  button.labelShadowOffset = new Point(-1, -1);//按钮图标定位
//  button.labelShadowColor = colors[1];
//  button.labelColor = this.buttonLabelColor;
//  button.contrast = this.buttonContrast;//对比
//  button.drawNew();
//  // button.hint = 'stage size\nsmall & normal';
//  button.fixLayout();//修复布局
//  button.refresh();//刷新
//  stageSizeButton = button;
//  this.controlBar.add(stageSizeButton);
//  this.controlBar.stageSizeButton = button; // for refreshing 为刷新
    ...
    button.labelMinExtent = new Point(36, 18);//大小
    button.padding = 0;
    button.labelShadowOffset = new Point(0, 0);//按钮图标定位
    button.labelShadowColor = colors[1];
    button.labelColor = this.buttonLabelColor;
    button.contrast = this.buttonContrast;//对比
    button.drawNew();
    ...
    this.controlBar.saveButton = button; // for refreshing 为刷新
    ...
    button.labelMinExtent = new Point(36, 18);//大小
    button.padding = 0;
    button.labelShadowOffset = new Point(-0.5, 0);//按钮图标定位
    button.labelShadowColor = colors[1];
    button.labelColor = this.buttonLabelColor;
    button.contrast = this.buttonContrast;//对比
    button.drawNew();
    ...
    button.refresh();//刷新
    openButton = button;
    this.controlBar.add(openButton);
    this.controlBar.openButton = button; // for refreshing 为刷新
	...
	button.corner = 12;
    button.color = new Color(74,108,212);
    button.highlightColor = colors[1];
    button.pressColor = colors[2];
    button.labelMinExtent = new Point(36, 18);//大小
    button.padding = 0;
    ...
    this.controlBar.submitButton = button; // for refreshing 为刷新
    ...
    button = new ToggleButtonMorph(
        ...
        'toggleAppMode',
        [
            new SymbolMorph('fullScreen', 14),//全屏
            new SymbolMorph('normalScreen', 14)//默认
        ],
        function () {  // query
            return myself.isAppMode;
        }
    ...
    button.highlightColor = colors[1];
    button.pressColor = colors[2];
    button.labelMinExtent = new Point(36, 25);//按钮大小
    button.padding = 0;
...
//      [
//          new SymbolMorph('footprints', 16),//足迹
...
//      'stopAllScripts',
//      [
//          new SymbolMorph('octagon', 14),//八角形
//          new SymbolMorph('square', 14)//正方形
//      ],
//      function () {  // query
//          return myself.stage ?
...
//  // startButton 启动按钮
    ...
    slider.alpha = 0.3;
    slider.setExtent(new Point(50, 25));
...
//  this.controlBar.cloudButton = cloudButton; // for menu positioning 为菜单定位

    this.controlBar.fixLayout = function () {
        x =  myself.right() -(3 * padding +  settingsButton.width());
...
//      slider.setRight(stageSizeButton.left() - padding);
        ...
        openButton.setCenter(myself.controlBar.center());
		openButton.setLeft(this.left());
        
		saveButton.setCenter(myself.controlBar.center());
		saveButton.setRight(this.left()-padding);
		
//      steppingButton.setCenter(myself.controlBar.center());
//      steppingButton.setRight(slider.left() - padding);
		startButton.setCenter(myself.controlBar.center());
		startButton.setRight(x-2*settingsButton.width()-padding);
		
        settingsButton.setCenter(myself.controlBar.center());
        settingsButton.setLeft(openButton.right()+padding);

		submitButton.setCenter(myself.controlBar.center());
		submitButton.setRight(this.right()-padding);

//      cloudButton.setCenter(myself.controlBar.center());
//      cloudButton.setRight(saveButton.left() - padding);

        projectButton.setCenter(myself.controlBar.center());
        projectButton.setRight(saveButton.left() - padding);

        this.refreshSlider();
        this.updateLabel();
    };

    this.controlBar.refreshSlider = function () {//刷新滑块
    ...
    this.controlBar.refreshResumeSymbol = function () {//刷新简历象征
        var pauseSymbols;
...
//      pauseButton.refresh();
    ...
    this.controlBar.updateLabel = function () {//更新标签
...
IDE_Morph.prototype.createCategories = function () {//创建类别
    var myself = this;
    ...
    function addCategoryButton(category) {//添加类别按钮
        var labelWidth = 175,
            colors = [
                myself.frameColor,
        ...
        button = new ToggleButtonMorph(//开关按钮变形
            colors,
            myself, // the IDE is the target
            function () {
    ...
    function fixCategoriesLayout() {//解决分类布局
        var buttonWidth = myself.categories.children[0].width(),
            buttonHeight = myself.categories.children[0].height(),
            border = 3,
            rows =  Math.ceil((myself.categories.children.length) ),
...
IDE_Morph.prototype.createPalette = function (forSearching) {//创建调色板
    // assumes that the logo pane has already been created 假设已经创建了徽标面板
    // needs the categories pane for layout 需要用于布局的类别面板
    var myself = this;
    ...
    if (forSearching) {
        this.palette = new ScrollFrameMorph(//滚动框变形
            null,
            null,
...
IDE_Morph.prototype.createPaletteHandle = function () {//创建面板处理
    // assumes that the palette has already been created  假设面板已经创建了
...
IDE_Morph.prototype.createStage = function () {//创建阶段
    // assumes that the logo pane has already been created 假设已经创建了logo面板
    if (this.stage) {this.stage.destroy(); }
    StageMorph.prototype.frameRate = 0;
    this.stage = new StageMorph(this.globalVariables);
    this.stage.setExtent(this.stage.dimensions); // dimensions are fixed 尺寸是固定的
...
IDE_Morph.prototype.createStageHandle = function () {//创建阶段处理
...
IDE_Morph.prototype.createSpriteBar = function () {//创建精灵
     // assumes that the categories pane has already been created//假设已经创建了类别面板
...
//  function addRotationStyleButton(rotationStyle) {//添加旋转样式按钮
...
//  }
    ...
    tabBar.tabTo = function (tabString) {
        ...
        active.refresh(); // needed when programmatically tabbing 时需要以编程方式指定
...
IDE_Morph.prototype.createSpriteEditor = function () {//创建精灵编辑器
    // assumes that the logo pane and the stage have already been created 假设已经创建了logo面板和舞台
    var scripts = this.currentSprite.scripts,txt
        myself = this;
...
//  	txt = new TextMorph(localize('1616'));
//	    txt.fontSize = 15;
//	    txt.setColor(SpriteMorph.prototype.paletteTextColor);
//	    txt.setPosition(new Point(100, 100));
//	    this.add(this.txt);
...
//  this.corralBar.setHeight(this.logo.height()); // height is fixed 高度是固定的
//  this.add(this.corralBar);

    // new sprite button  新的精灵按钮
...
//  newbutton.drawNew();
//  newbutton.hint = "Start";//提示
//  newbutton.fixLayout();
//  newbutton.setCenter(this.center());
//  newbutton.setTop(this.logo.height());
//  newbutton.setLeft(this.logo.width());
//  this.add(newbutton);
...
//  }
        ...
        this.spriteEditor.growth = 50;//增长
        this.spriteEditor.isDraggable = false;//是否可拖动
...
//IDE_Morph.prototype.createCorralBar = function () {//创建控制栏
//  // assumes the stage has already been created  假设舞台已经被创建了  
//  var padding = 5,
//      newbutton,//创建按钮
//      paintbutton,//油漆按钮
//      cambutton,     //凸轮按钮
...
//  newbutton.contrast = this.buttonContrast;
//  newbutton.drawNew();
//  newbutton.hint = "add a new Turtle sprite";//提示
//  newbutton.fixLayout();
...
//      'stopAllScripts',
//      [
//          new SymbolMorph('octagon', 25),//八角形
//          new SymbolMorph('square', 25)//正方形
//      ],
//      function () {  // query
//          return myself.stage ?
//                  myself.stage.enableCustomHatBlocks &&
...
//  stopButton = button;
//  this.corralBar.add(stopButton);
//  this.corralBar.stopButton = stopButton; // for refreshing
//
//  //pauseButton 暂停按钮
//  button = new ToggleButtonMorph(
//      null, //colors,
...
//      'togglePauseResume',
//      [
//          new SymbolMorph('pause', 25),
//          new SymbolMorph('pointRight', 25)
//      ],
//      function () {  // query
//          return myself.isPaused();
//      }
...
//  button.refresh();
//  button.setCenter(this.corralBar.center());
//  button.setLeft(this.corralBar.left() + padding + stopButton.width() + padding);
//  pauseButton = button;
//  this.corralBar.add(pauseButton);
//  this.corralBar.pauseButton = pauseButton; // for refreshing
//
//  // startButton 启动按钮
//  button = new PushButtonMorph(
//      this,
...
//  button.fixLayout();
//  button.setCenter(this.corralBar.center());
//  button.setLeft( this.corralBar.left() +
//          padding +
//          stopButton.width() +
//          padding +
//          pauseButton.width() +
//          padding);
//  startButton = button;
//  this.corralBar.add(startButton);
//  this.corralBar.startButton = startButton;
//
//}

IDE_Morph.prototype.createCorral = function () {//创建控制
    // assumes the corral bar has already been created
    var frame, txt,template, padding = 5, myself = this,nr;
    ...
    this.corral.destroy();
    this.corral = new FrameMorph(this);
    this.corral.acceptsDrops = false;
    this.corral.reactToDropOf = function (icon) {
        myself.reactToDropOf(icon);
    };
    this.addBack(this.corral);
    
//	if(sessionStorage.getItem('xszl')==undefined || sessionStorage.getItem('xszl')==null){
//		nr='';
//	}else{
//		var str=sessionStorage.getItem('xszl').replace(/&&/g, '\n');
//		console.log(str);
//		nr=str;
//	}
//	txt = new TextMorph(localize(nr));
//  txt.fontSize = 15;
//  txt.setColor(SpriteMorph.prototype.paletteTextColor);
//  txt.setPosition(new Point(localStorage.getItem('wzx'), localStorage.getItem('wzy')));
//  this.corral.txt=txt;
//  this.corral.add(this.corral.txt);
...
//  	this.txt.setLeft(this.left() + padding);
...
//  this.corral.arrangeIcons = function () {//排列图标
...
//          if (x + w > max) {
//              x = start;
//              y += icon.height(); // they're all the same  他们都是相同的
//          }
...
//  this.corral.addSprite = function (sprite) {//添加精灵
...
//	this.corral.txt.refresh= function(){
//		alert('123')
//		txt = new TextMorph(localize(localStorage.getItem('zl')));
//	    txt.fontSize = 15;
//	    txt.setColor(SpriteMorph.prototype.paletteTextColor);
//	    txt.setPosition(new Point(localStorage.getItem('wzx'), localStorage.getItem('wzy')));
//	}
...
//IDE_Morph.prototype.createCorralright=function(){
//	 this.createCorralright = new Morph();
//  this.createCorralright.color = this.frameColor;
//  this.createCorralright.setHeight(this.corral.height()); // height is fixed 高度是固定的
//  this.add(this.corralright);
//}
// IDE_Morph layout
...
IDE_Morph.prototype.fixLayout = function (situation) {
    ...
    if (situation !== 'refreshPalette') {
        ...
        if (this.isAppMode) {
            ...
            //debugger;
            this.stage.setTop(this.bottom()-this.stage.height());
            //console.log(this.bottom());
            this.stage.setLeft(this.width()-this.stage.width());
		...
		document.getElementById('jb').style.width=this.stage.width()+'px';
		var w = document.documentElement.clientWidth || document.body.clientWidth;
		var h = document.documentElement.clientHeight || document.body.clientHeight;
		var he=h-this.logo.height()-this.stage.height();
		document.getElementById('jb').style.height=he+'px';
		document.getElementById('jb').innerHTML=sessionStorage.getItem('xszl').replace(/&&/g, '</br>');
        // spriteBar
...
//      this.corralBar.setBottom(this.stage.top());
//      this.corralBar.setWidth(this.stage.width());
//    	this.corralBar.setHeight(this.logo.height());

        // corral
//      if (!contains(['selectSprite', 'tabEditor'], situation)) {
            this.corral.setPosition(this.stage.topLeft());
            this.corral.setTop(this.logo.bottom()+padding);
            this.corral.setWidth(this.stage.width());
            this.corral.setHeight(this.height() - this.stage.height()-this.logo.height()-padding*2);
            localStorage.setItem('wzx',this.width()-this.stage.width()+padding*2);
			localStorage.setItem('wzy',padding*5);
            this.corral.fixLayout();
//      }
		
        //corralright
        //this.corralright.setBottom(this.corral.right());
        //this.corralright.setWidth(this.stage.width()/2);
    }
...
IDE_Morph.prototype.setProjectName = function (string) {//设置项目名称
    this.projectName = string.replace(/['"]/g, ''); // filter quotation marks  过滤器引号
    this.hasChangedMedia = true;
    this.controlBar.updateLabel();
};

// IDE_Morph resizing  调整

IDE_Morph.prototype.setExtent = function (point) {//设置范围
    ...
    // determine the minimum dimensions making sense for the current mode 确定当前模式的最小维度
    if (this.isAppMode) {
    ...
    // adjust stage ratio if necessary 必要时调整阶段比率
    maxWidth = ext.x -
...
IDE_Morph.prototype.reactToWorldResize = function (rect) {//应对世界调整
    if (this.isAutoFill) {
...
IDE_Morph.prototype.droppedImage = function (aCanvas, name) {//形象下降
...
IDE_Morph.prototype.droppedBinary = function (anArrayBuffer, name) {
    // dynamically load ypr->Snap!  动态的加载
...
// IDE_Morph button actions  按钮操作

IDE_Morph.prototype.refreshPalette = function (shouldIgnorePosition) {//刷新面板
...
IDE_Morph.prototype.pressStart = function () {//按下开始
	var date='run';
	http.get('http://127.0.0.1:3000/'+date,function(err,result){
		});
//  if (this.world().currentKey === 16) { // shiftClicked 移点击
...
IDE_Morph.prototype.toggleFastTracking = function () {//切换快速跟进
...
IDE_Morph.prototype.toggleVariableFrameRate = function () {//切换可变帧率
...
IDE_Morph.prototype.toggleSingleStepping = function () {//切换单步进
    this.stage.threads.toggleSingleStepping();
    this.controlBar.steppingButton.refresh();
    this.controlBar.refreshSlider();
};

IDE_Morph.prototype.toggleCameraSupport = function () {//切换相机支持
...
IDE_Morph.prototype.startFastTracking = function () {//开始快速跟进
...
IDE_Morph.prototype.stopFastTracking = function () {//停止快速跟进
...
IDE_Morph.prototype.runScripts = function () {//运行脚本
    this.stage.fireGreenFlagEvent();
};

IDE_Morph.prototype.togglePauseResume = function () {//切换暂停恢复
...
IDE_Morph.prototype.isPaused = function () {//暂停
    if (!this.stage) {return false; }
    return this.stage.threads.isPaused();
};

IDE_Morph.prototype.stopAllScripts = function () {//停止所有的脚本
...
IDE_Morph.prototype.selectSprite = function (sprite) {//选择精灵
...
// IDE_Morph retina display support 视网膜显示支持

IDE_Morph.prototype.toggleRetina = function () {//切换视网膜
    if (isRetinaEnabled()) {
...
IDE_Morph.prototype.defaultDesign = function () {//默认设计
    this.setDefaultDesign();
    this.refreshIDE();
    this.removeSetting('design');
};

IDE_Morph.prototype.flatDesign = function () {//平面设计
    this.setFlatDesign();
    this.refreshIDE();
    this.saveSetting('design', 'flat');
};

IDE_Morph.prototype.refreshIDE = function () {//刷新IDE
    var projectData;
...
// IDE_Morph settings persistance 设置暂留

IDE_Morph.prototype.applySavedSettings = function () {//应用保存设置
    ...
    // nested auto-wrapping 嵌套auto-wrapping
    ...
    // plain prototype labels 普通的原型标签
    if (plainprototype) {
        BlockLabelPlaceHolderMorph.prototype.plainLabel = true;
    }
};

IDE_Morph.prototype.saveSetting = function (key, value) {//保存设置
...
IDE_Morph.prototype.getSetting = function (key) {//得到设置
    if (this.hasLocalStorage()) {
...
IDE_Morph.prototype.removeSetting = function (key) {//删除设置
    if (this.hasLocalStorage()) {
        delete localStorage['-snap-setting-' + key];
    }
};

IDE_Morph.prototype.hasLocalStorage = function () {//有本地存储
	// checks whether localStorage is available, 检查localStorage是否可用，
    // this kludgy try/catch mechanism is needed 这种组合/捕获机制是需要的
    // because Safari 11 is paranoid about accessing 因为Safari 11对访问很偏执
    // localstorage from the file:// protocol  来自文件：//协议的localstorage
	try { 
...
// IDE_Morph sprite list access 精灵访问列表

IDE_Morph.prototype.addNewSprite = function () {//添加新的精灵
    ...
    // randomize sprite properties 随机精灵属性
...
IDE_Morph.prototype.paintNewSprite = function () {//涂料新精灵
    var sprite = new SpriteMorph(this.globalVariables),
        cos = new Costume(),
        myself = this;
...
IDE_Morph.prototype.newCamSprite = function () {//新凸轮精灵
    var sprite = new SpriteMorph(this.globalVariables),
        camDialog,
        myself = this;
    ...
    camDialog = new CamSnapshotDialogMorph(//凸轮快照对话框变形
        this,
        sprite,
        function () { myself.removeSprite(sprite); },
        function (costume) {
...
IDE_Morph.prototype.duplicateSprite = function (sprite) {//重复的精灵
...
IDE_Morph.prototype.instantiateSprite = function (sprite) {//实例化精灵
...
IDE_Morph.prototype.removeSprite = function (sprite) {//删除精灵
    var idx, myself = this;
...
IDE_Morph.prototype.newSpriteName = function (name, ignoredSprite) {//新精灵的名字
    var ix = name.indexOf('('),
...
// IDE_Morph deleting scripts 删除脚本
...
IDE_Morph.prototype.projectMenu = function () {// 文件按钮
    var menu,
        myself = this,
        world = this.world(),
...
IDE_Morph.prototype.resourceURL = function () {//资源URL
    // Take in variadic inputs that represent an a nested folder structure. 使用可变的输入表示一个嵌套的文件夹结构。
    // Method can be easily overridden if running in a custom location.  如果在一个定制的位置运行，那么方法很容易被覆盖。
    // Default Snap! simply returns a path (relative to snap.html)  简单地返回一条路径（相对于snaphtml）
    var args = Array.prototype.slice.call(arguments, 0);
    return args.join('/');
};

IDE_Morph.prototype.getMediaList = function (dirname, callback) {//媒体列表
...
IDE_Morph.prototype.parseResourceFile = function (text) {//解析资源文件
    // A Resource File lists all the files that could be loaded in a submenu  资源文件列出了可以在子菜单中加载的所有文件
    // Examples are libraries/LIBRARIES, Costumes/COSTUMES, etc 例子是图书馆/图书馆、服装/服装等。
    // The file format is tab-delimited, with unix newlines:  //文件格式是用制表符分隔的，使用unix新行：
    // file-name, Display Name, Help Text (optional)  文件名称、显示名称、帮助文本（可选）
    var parts,
        items = [];
...
IDE_Morph.prototype.importMedia = function (folderName) {//导入媒体
...
IDE_Morph.prototype.popupMediaImportDialog = function (folderName, items) {//弹出媒体导入对话框
...
// IDE_Morph menu actions  菜单操作

IDE_Morph.prototype.aboutSnap = function () {//关于snap
...
//  noticeTxt = localize('License')//许可证
//      + '\n\n'
//      + 'Snap! is free software: you can redistribute it and/or modify\n'
//      + 'it under the terms of the GNU Affero General Public License as\n'
//      + 'published by the Free Software Foundation, either version 3 of\n'
...
//
//  creditsTxt = localize('Contributors')//贡献者
...
IDE_Morph.prototype.editProjectNotes = function () {//编辑项目笔记
...
IDE_Morph.prototype.newProject = function () {//新项目
...
IDE_Morph.prototype.save = function () {
    if (this.source === 'examples') {
        this.source = 'local'; // cannot save to examples  不能保存的例子
    } 
    if (this.projectName) {
...
//console.log(localStorage.getItem('-snap-project-654161'));
// Serialize a project and save to the browser. 序列化一个项目并保存到浏览器。
...
IDE_Morph.prototype.exportProject = function (name, plain) {//导出项目
...
IDE_Morph.prototype.exportGlobalBlocks = function () {//导出区域块
...
IDE_Morph.prototype.removeUnusedBlocks = function () {//删除未使用的块
...
IDE_Morph.prototype.exportSprite = function (sprite) {//导出精灵
...
IDE_Morph.prototype.exportScriptsPicture = function () {//导出脚本图片
    var pics = [],
        pic,
        padding = 20,
        w = 0,
    ...
    // collect global block pics 收集全球块图片
    ...
    // determine dimensions of composite 确定的尺寸组合
...
IDE_Morph.prototype.exportProjectSummary = function (useDropShadows) {//导出项目总结
    ...
    function addVariables(varFrame) {//添加变量
    ...
    function addBlocks(definitions) {//添加模块
...
IDE_Morph.prototype.openProjectString = function (str) {//开放项目字符串
    var msg,
        myself = this;
    this.nextSteps([
        function () {
...
IDE_Morph.prototype.rawOpenProjectString = function (str) {//原始字符串开放项目
...
IDE_Morph.prototype.openCloudDataString = function (str) {//开放云数据字符串
    var msg,
        myself = this,
...
IDE_Morph.prototype.rawOpenCloudDataString = function (str) {//原始开放云数据字符串
    var model;
    StageMorph.prototype.hiddenPrimitives = {};
    StageMorph.prototype.codeMappings = {};
    StageMorph.prototype.codeHeaders = {};
...
IDE_Morph.prototype.openBlocksString = function (str, name, silently) {//开放的街区字符串
    var msg,
        myself = this;
    this.nextSteps([
        function () {
...
IDE_Morph.prototype.openSpritesString = function (str) {//打开精灵字符串
    var msg,
        myself = this;
    this.nextSteps([
        function () {
...
IDE_Morph.prototype.switchToUserMode = function () {//切换到用户模式
...
IDE_Morph.prototype.switchToDevMode = function () {//切换到开发模式
    var world = this.world();
...
IDE_Morph.prototype.flushBlocksCache = function (category) {//冲洗块缓存
...
IDE_Morph.prototype.flushPaletteCache = function (category) {//刷新面板缓存
    // if no category is specified, the whole cache gets flushed
    if (category) {
...
IDE_Morph.prototype.toggleZebraColoring = function () {//切换斑马着色
    var scripts = [];
    ...
    // force-update all scripts: force-update所有脚本:
    scripts.forEach(function (topBlock) {
        topBlock.fixBlockColor(null, true);
    });
};

IDE_Morph.prototype.toggleDynamicInputLabels = function () {//切换动态输入标签
    var projectData;
    SyntaxElementMorph.prototype.dynamicInputLabels =
        !SyntaxElementMorph.prototype.dynamicInputLabels;
    if (Process.prototype.isCatchingErrors) {
...
IDE_Morph.prototype.toggleBlurredShadows = function () {//切换模糊阴影
    window.useBlurredShadows = !useBlurredShadows;
};

IDE_Morph.prototype.toggleLongFormInputDialog = function () {//切换长表单输入对话框
    InputSlotDialogMorph.prototype.isLaunchingExpanded =
        !InputSlotDialogMorph.prototype.isLaunchingExpanded;
...
IDE_Morph.prototype.togglePlainPrototypeLabels = function () {//切换平原原型标签
    BlockLabelPlaceHolderMorph.prototype.plainLabel =
        !BlockLabelPlaceHolderMorph.prototype.plainLabel;
...
IDE_Morph.prototype.togglePreferEmptySlotDrops = function () {//切换选择空槽
    ScriptsMorph.prototype.isPreferringEmptySlots =
        !ScriptsMorph.prototype.isPreferringEmptySlots;
};

IDE_Morph.prototype.toggleVirtualKeyboard = function () {//切换虚拟键盘
    MorphicPreferences.useVirtualKeyboard =
        !MorphicPreferences.useVirtualKeyboard;
};

IDE_Morph.prototype.toggleInputSliders = function () {//切换输入滑块
    MorphicPreferences.useSliderForInput =
        !MorphicPreferences.useSliderForInput;
};

IDE_Morph.prototype.toggleSliderExecute = function () {//拨动滑块执行
    ArgMorph.prototype.executeOnSliderEdit =
        !ArgMorph.prototype.executeOnSliderEdit;
};

IDE_Morph.prototype.toggleAppMode = function (appMode) {//App模式开关
    var world = this.world(),
        elements = [
            this.logo,
...
//          this.controlBar.steppingButton,
            ...
            this.controlBar.saveButton,
            this.controlBar.startButton,
            this.controlBar.openButton,
            this.controlBar.submitButton,
            this.paletteHandle,
            this.stageHandle,
//          this.corral,
//          this.corralBar,
...
IDE_Morph.prototype.toggleStageSize = function (isSmall, forcedRatio) {//切换阶段大小
    var myself = this,
...
IDE_Morph.prototype.toggleSave=function(){
	if (this.source === 'examples') {
        this.source = 'local'; // cannot save to examples  不能保存的例子
    } 
    if (this.projectName) {
...
IDE_Morph.prototype.toggleSubmit=function(){
	http.get('http://http://127.0.0.1:3000/start',function(err,result){});
	num=0;
	setTimeout('dy()',1000);
}

function dy(){
	if(sessionStorage.getItem('xszl')!=null && sessionStorage.getItem('xszl') !=undefined){
		var a=sessionStorage.getItem('xszl');
...
function getDetail(date) {
    if(num<date.length){  
        setTimeout(function(){  
            http.get('http://http://127.0.0.1:3000/'+date[num],function(err,result){});
            num++;
            getDetail(date);  
        },10);  
    }
...
IDE_Morph.prototype.toggleOpen=function(){
	new ProjectDialogMorph(this, 'open').popUp();
}


IDE_Morph.prototype.setPaletteWidth = function (newWidth) {//设置面板宽度
...
IDE_Morph.prototype.createNewProject = function () {//创建新项目
    var myself = this;
    this.confirm(
...
// IDE_Morph localization 本地化
 
IDE_Morph.prototype.languageMenu = function () {//语言菜单
    var menu = new MenuMorph(this),
        world = this.world(),
        pos = this.controlBar.settingsButton.bottomLeft(),
        myself = this;
...
IDE_Morph.prototype.setLanguage = function (lang, callback) {//设置语言
...
IDE_Morph.prototype.reflectLanguage = function (lang, callback) {//表达语言
    var projectData,
...
// IDE_Morph blocks scaling 块扩展

IDE_Morph.prototype.userSetBlocksScale = function () {//使用资源集块规模
    var myself = this,
        scrpt,
        blck,
        shield,
...
IDE_Morph.prototype.setBlocksScale = function (num) {//组块规模
    var projectData;
    if (Process.prototype.isCatchingErrors) {
        try {
            projectData = this.serializer.serialize(this.stage);
...
// IDE_Morph stage size manipulation 舞台大小操纵
...
IDE_Morph.prototype.setStageExtent = function (aPoint) {//设置阶段程度
    var myself = this,
        world = this.world(),
...
// IDE_Morph dragging threshold (internal feature) 拖动阈值(内部特性)
...
IDE_Morph.prototype.createCloudAccount = function () {//创建云帐户
    var myself = this,
        world = this.world();
/*
...
IDE_Morph.prototype.resetCloudPassword = function () {//云密码重置
    var myself = this,
        world = this.world();
/*
    // force-logout, commented out for now:
...
IDE_Morph.prototype.changeCloudPassword = function () {//改变云密码
    var myself = this,
        world = this.world();
    new DialogBoxMorph(
        null,
...
IDE_Morph.prototype.logout = function () {//注销
    var myself = this;
    delete localStorage['-snap-user'];
    SnapCloud.logout(
        function () {
...
IDE_Morph.prototype.saveProjectToCloud = function (name) {//将项目保存到云
    var myself = this;
    if (name) {
...
IDE_Morph.prototype.exportProjectMedia = function (name) {//导出项目的媒体
    var menu, media;
...
IDE_Morph.prototype.exportProjectNoMedia = function (name) {//导出项目没有媒体
    var menu, str;
    this.serializer.isCollectingMedia = true;
    if (name) {
        this.setProjectName(name);
...
IDE_Morph.prototype.exportProjectAsCloudData = function (name) {//导出项目为云数据
...
IDE_Morph.prototype.cloudAcknowledge = function () {//云承认
    var myself = this;
...
IDE_Morph.prototype.cloudResponse = function () {//云的反应
    var myself = this;
    return function (responseText, url) {
...
IDE_Morph.prototype.cloudError = function () {//云计算错误
    var myself = this;
...
IDE_Morph.prototype.cloudIcon = function (height, color) {//云图标
...
IDE_Morph.prototype.setCloudURL = function () {//集云URL
    new DialogBoxMorph(
        null,
        function (url) {
            SnapCloud.url = url;
...
// IDE_Morph HTTP data fetching HTTP数据抓取

IDE_Morph.prototype.getURL = function (url, callback) {//获取URL
...
IDE_Morph.prototype.showMessage = function (message, secs) {//显示消息
...
IDE_Morph.prototype.inform = function (title, message) {//通知
    new DialogBoxMorph().inform(
        title,
        localize(message),
        this.world()
...
IDE_Morph.prototype.confirm = function (message, title, action) {//确认
...
IDE_Morph.prototype.prompt = function (message, callback, choices, key) {//提示
...
// ProjectDialogMorph inherits from DialogBoxMorph: 项目对话框从对话框中继承：
...
ProjectDialogMorph.prototype.init = function (ide, task) {//项目对话框原型初始化
    var myself = this;
    ...
    // initialize inherited properties: 初始化继承的属性:
    ...
    // override inherited properites: 覆盖继承在弹出的菜单:
...
ProjectDialogMorph.prototype.popUp = function (wrrld) {//弹出
...
// ProjectDialogMorph source buttons 项目对话框变形源按钮
...
// ProjectDialogMorph list field control 项目对话框变形列表字段控制

ProjectDialogMorph.prototype.fixListFieldItemColors = function () {
    // remember to always fixLayout() afterwards for the changes 记住，在以后的修改中始终记得始终固定布局（）
    // to take effect 生效
    var myself = this;
...
// ProjectDialogMorph filter field 项目对话框变形筛选字段
...
// ProjectDialogMorph ops  项目对话框变形行动
...
ProjectDialogMorph.prototype.setSource = function (source) {
    ...
    } else { // 'examples'; 'cloud' is initialized elsewhere  “云”初始化
        this.listField.action = function (item) {
            var src, xml;
            if (item === undefined) {return; }
            if (myself.nameField) {
...
ProjectDialogMorph.prototype.getLocalProjectList = function () {//得到当地项目列表
    var stored, name, dta,
        projects = [];
...
ProjectDialogMorph.prototype.getExamplesProjectList = function () {//示例项目列表
    return this.ide.getMediaList('Examples');
};

ProjectDialogMorph.prototype.installCloudProjectList = function (pl) {//安装云项目列表
    var myself = this;
    this.projectList = pl || [];
    ...
    this.listField = new ListMorph(
        ...
        [ // format: display shared project names bold 格式：显示共享项目名称粗体
            [
                'bold',
...
ProjectDialogMorph.prototype.clearDetails = function () {//清除细节
    this.notesText.text = '';
...
ProjectDialogMorph.prototype.openProject = function () {//打开项目
    ...
    if (this.source === 'cloud') {
        ...
        // Note "file" is a property of the parseResourceFile function. 注意“File”是解析资源文件函数的属性。
...
// ProjectDialogMorph layout 项目对话框改变布局
...
LibraryImportDialogMorph.prototype.init = function (ide, librariesData) {//库导入对话框变形
    // initialize inherited properties:
...
LibraryImportDialogMorph.prototype.initializePalette = function () {//初始化面板
...
LibraryImportDialogMorph.prototype.initializeLibraryDescription = function () {//初始化库描述
...
LibraryImportDialogMorph.prototype.installLibrariesList = function () {//安装库列表
    var myself = this;
...
LibraryImportDialogMorph.prototype.popUp = function () {//弹出
...
// Library Cache Utilities. 库缓存实用程序。
...
LibraryImportDialogMorph.prototype.displayBlocks = function (libraryKey) {//显示模块
    ...
    // populate palette, grouped by categories. 填充调色板，按类别分组。
    this.initializePalette();
...
// SpriteIconMorph inherits from ToggleButtonMorph (Widgets) 从开关按钮变形（窗口小部件）
...
// SpriteIconMorph settings 精灵图标改变设置
...
// SpriteIconMorph instance creation: 精灵图标变形实例创建：
...
SpriteIconMorph.prototype.init = function (aSprite, aTemplate) {
    ...
    action = function () {
        // make my sprite the current one 让我的精灵成为当前的精灵
    ...
    query = function () {
        // answer true if my sprite is the current one  如果精灵是当前的精灵，答案是正确的
        var ide = myself.parentThatIsA(IDE_Morph);
    ...
    // override defaults and build additional components 覆盖默认值并构建其他组件
    this.isDraggable = true;
...
SpriteIconMorph.prototype.createThumbnail = function () {//创建缩略图
    if (this.thumbnail) {
    ...
    if (this.object instanceof SpriteMorph) { // support nested sprites 支持嵌套的精灵
...
SpriteIconMorph.prototype.createLabel = function () {//创建标签
    var txt;
...
//SpriteIconMorph.prototype.createRotationButton = function () {//创建旋转按钮
...
// SpriteIconMorph stepping 精灵图标改变

SpriteIconMorph.prototype.step = function () {//步
...
SpriteIconMorph.prototype.duplicateSprite = function () {//重复的精灵
...
SpriteIconMorph.prototype.instantiateSprite = function () {//实例化精灵
    var ide = this.parentThatIsA(IDE_Morph);
    if (ide) {
...
SpriteIconMorph.prototype.removeSprite = function () {//删除精灵
    var ide = this.parentThatIsA(IDE_Morph);
    if (ide) {
...
SpriteIconMorph.prototype.exportSprite = function () {//导出精灵
    this.object.exportSprite();
};

SpriteIconMorph.prototype.chooseExemplar = function () {//选择的范例
    this.object.chooseExemplar();
};

SpriteIconMorph.prototype.releaseSprite = function () {//释放精灵
    this.object.release();
};

SpriteIconMorph.prototype.showSpriteOnStage = function () {//精灵在舞台上展示
    this.object.showOnStage();
};

// SpriteIconMorph events 精灵图标改变事件

SpriteIconMorph.prototype.mouseDoubleClick = function () {//鼠标双击
...
SpriteIconMorph.prototype.createBackgrounds = function () {//创建背景
//    only draw the edges if I am selected 如果我被选中，只画边
    var context, 
        ext = this.extent();

    if (this.template) { // take the backgrounds images from the template 从模板中获取背景图像
...
// SpriteIconMorph drag & drop 精灵图标变形拖放

SpriteIconMorph.prototype.prepareToBeGrabbed = function () {//准备了
    var ide = this.parentThatIsA(IDE_Morph),
        idx;
...
SpriteIconMorph.prototype.wantsDropOf = function (morph) {
    // allow scripts & media to be copied from one sprite to another 允许脚本和媒体从一个精灵复制到另一个
    // by drag & drop 通过拖放
    return morph instanceof BlockMorph
...
SpriteIconMorph.prototype.copyStack = function (block) {//复制栈
...
CostumeIconMorph.prototype.createThumbnail = function () {//创建缩略图
    var txt;
...
CostumeIconMorph.prototype.userMenu = function () {//用户菜单
    var menu = new MenuMorph(this);
...
CostumeIconMorph.prototype.editCostume = function () {//编辑Costume
    this.disinherit();
    if (this.object instanceof SVG_Costume) {
...
CostumeIconMorph.prototype.editRotationPointOnly = function () {//只编辑旋转点
    var ide = this.parentThatIsA(IDE_Morph);
    this.object.editRotationPointOnly(this.world());
    ide.hasChangedMedia = true;
};

CostumeIconMorph.prototype.renameCostume = function () {//重命名的Costume
    this.disinherit();
...
CostumeIconMorph.prototype.duplicateCostume = function () {//重复的Costume
...
CostumeIconMorph.prototype.removeCostume = function () {//删除Costume
    var wardrobe = this.parentThatIsA(WardrobeMorph),
