...
var CustomBlockDefinition;//自定义块定义;
var CustomCommandBlockMorph;//自定义命令块变形;
var CustomReporterBlockMorph;//定制的记者块变形;
var BlockDialogMorph;//块对话框变形;
var BlockEditorMorph;//块编辑器变形;
var PrototypeHatBlockMorph;//原型的帽子块变形;
var BlockLabelFragment;//块标签片段;
var BlockLabelFragmentMorph;//块标签片段变形;
var BlockInputFragmentMorph;//块的输入段变形;
var BlockLabelPlaceHolderMorph;//块标签位置持有人变形;
var InputSlotDialogMorph;//输入对话框槽变形;
var VariableDialogMorph;//变量对话框变形;
var JaggedBlockMorph;//锯齿状的块变形;
var BlockExportDialogMorph;//块出口对话框变形;
var BlockImportDialogMorph;//块导入对话框变形;
var BlockRemovalDialogMorph;//移除对话框变形;
...
CustomCommandBlockMorph.prototype.refresh = function (aDefinition, silently) {
	...
	if(this.blockZl!==newZl){
		 this.setZl(newZl, silently, def);
	}
...
BlockEditorMorph.prototype.init = function (definition, target) {
    ...
    this.handle = null;
	if(definition.zl){
		sessionStorage.setItem('zdyzl',definition.zl);
	}else{
		sessionStorage.setItem('zdyzl','');
	}
    // initialize inherited properties:
...
BlockEditorMorph.prototype.order=function(){
	var e1 = document.getElementById('modal-overlay');
	var zdyzl=sessionStorage.getItem('zdyzl');
	document.getElementById('zdyzl').value=zdyzl;
...
BlockEditorMorph.prototype.updateDefinition = function () {
    ...
    this.definition.zl=sessionStorage.getItem('zdyzl');
